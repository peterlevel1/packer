;(function (root) {
  ...
  function tellAll(state, arr) {
    ...
    var results = [],
      allArePromise = arr.commitments.every(function (commitment, index) {
        if (isPromise(commitment)) {
          if (!commitment.isPending()) {
            ...
            if (state === 'resolve') {
              commitment.resolve(arr.values[index]);
            }
            else if (state === 'reject') {
              commitment.reject(arr.values[index]);
            }
            else if (state === 'notify' &&
              typeof commitment._promiseState.progress === 'function' ) {
              commitment.notify(arr.values[index]);
            }
          }
          return true;
        }
  ...
  function handleOnePromise(state, o, v) {
    if (!/^(resolve|reject|notify)$/.test(state)) {
      return false;
    }
